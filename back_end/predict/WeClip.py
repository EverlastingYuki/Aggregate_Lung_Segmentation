...
def predict_WeClip(project_root="", one_channel_dir="", three_channel_dir="", WeClip_dir=""):
    ...
    proj_r = current_dir[:-16]

    get_source_img_path = os.path.join(proj_r, three_channel_dir)

    get_fake_img_path = os.path.join(proj_r, "back_end\\WeClip\\fake_img")
    ...
    l1 = get_filenames_without_extension(get_source_img_path)
    l2 = get_filenames_without_extension(get_fake_img_path)


    for i in l1:
        if i in l2:
            shutil.copy(os.path.join(get_fake_img_path, i+".png"), output_dir)




if __name__ == '__main__':
    predict_WeClip(three_channel_dir="front_end/static/uploaded/three_channel", WeClip_dir="front_end/static/result/WeClip")
