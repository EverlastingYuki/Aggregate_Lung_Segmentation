import argparse
import os
os.chdir(r"C:\Users\q\Downloads\Aggregate_Lung_Segmentation\back_end\WeClip")
import sys
sys.path.append(".")
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        try:
            ...
            resized_segs = F.interpolate(segs, size=labels.shape[1:3], mode='bilinear', align_corners=False)
        ...
        try:
            ...
            resized_msc_segs = F.interpolate(msc_segs, size=labels.shape[1:3], mode='bilinear', align_corners=False)
...
def main(cfg):
    ...
    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=WeCLIP_model, dataset=val_dataset, test_scales=[1.0, 0.75])
